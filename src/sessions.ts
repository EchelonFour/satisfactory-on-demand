...
export function currentSessionCount$(readInterval = config.get('statsReadInterval'), envoyPort = config.get('envoyAdminPort')): Observable<number> {
  return interval(readInterval).pipe(
    switchMap(() => axios.get<EnvoyStatsResponse>(`http://localhost:${envoyPort}/stats?filter=udp.query.downstream_sess_active&format=json`)),
